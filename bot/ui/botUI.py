...
@dataclass
class BotUISelections:
    selected_exp: str | None
    selected_e2: str | None
    selected_heroes: str | None
    quest_enabled: bool
...
class BotUI:
    ...
    async def _get_driver(self):
        if self._driver is None:
            self._driver = await MyDriver().get_driver()
        return self._driver

    async def renderUI(self):
    ...
    async def make_postrefresh_action(self):
        driver = await self._get_driver()
        script = """
    ...
    async def get_selected_exp(self):
        driver = await self._get_driver()
        return await driver.evaluate("() => window.selectedExp")
 
    async def get_selected_elita2(self):
        driver = await self._get_driver()
        return await driver.evaluate("() => window.selectedE2")
 
    async def get_selected_heroes(self):
        driver = await self._get_driver()
        return await driver.evaluate("() => window.selectedHeroes")
 
    async def get_quest_enabled(self):
        driver = await self._get_driver()
        return await driver.evaluate("() => window.questEnabled")
 
    async def get_selections(self) -> BotUISelections:
        return BotUISelections(
            selected_exp=await self.get_selected_exp(),
            selected_e2=await self.get_selected_elita2(),
            selected_heroes=await self.get_selected_heroes(),
            quest_enabled=await self.get_quest_enabled(),
        )
 
    async def set_selected_exp_to_default(self):
        driver = await self._get_driver()
    ...
    async def set_selected_exp_to_new_value(self, value):
        driver = await self._get_driver()
        await driver.evaluate(
            f"""
    ...
    async def restore_UI(self, selected_e2):
        driver = await self._get_driver()
        await driver.evaluate(
        f"""
